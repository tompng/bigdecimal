...
class TestVpOperation < Test::Unit::TestCase
  ...
  def test_newton_inverse
    xs = [BigDecimal(3), BigDecimal('123e50'), BigDecimal('5' * 89), BigDecimal('6' * 90), BigDecimal('7' * 91)]
    [*1..32, 50, 100, 200, 300].each do |prec|
      xs.each do |x|
        inv = x.newton_raphson_inverse(prec)
        inv_expected = 10 ** (prec + x.exponent) / x.to_i
        assert_in_delta(inv_expected, inv, 1)
      end
    end
  end
