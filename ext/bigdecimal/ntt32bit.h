#define NTT_FUNCNAME(name) name##32
#define UINT uint32_t
#define ADD_MULT_MOD(a, b, c, mod) (((a) + (uint64_t)(b) * (c)) % mod)
#define MULT_MOD(a, b, mod) (((uint64_t)(a) * (b)) % (mod))
#include "ntt.h"
#undef UINT
#undef NTT_FUNCNAME
#undef ADD_MULT_MOD
...
#define NTT32_DECDIG_BASE 1000000000
...
mod_restore_prime_24_26_29_shift_27(uint32_t mod1, uint32_t mod2, uint32_t mod3, uint32_t *digits) {
    ...
    c2 %= NTT32_PRIME3;
    // Base conversion
    c1 += c2 % NTT32_DECDIG_BASE * NTT32_PRIME2;
    c0 += c1 % NTT32_DECDIG_BASE * NTT32_PRIME1;
    c1 /= NTT32_DECDIG_BASE;
    digits[0] = c0 % NTT32_DECDIG_BASE;
    c0 /= NTT32_DECDIG_BASE;
    c1 += c2 / NTT32_DECDIG_BASE % NTT32_DECDIG_BASE * NTT32_PRIME2;
    c0 += c1 % NTT32_DECDIG_BASE * NTT32_PRIME1;
    c1 /= NTT32_DECDIG_BASE;
    digits[1] = c0 % NTT32_DECDIG_BASE;
    digits[2] = (uint32_t)(c0 / NTT32_DECDIG_BASE + c1 % NTT32_DECDIG_BASE * NTT32_PRIME1);
}
...
ntt_multiply32(uint32_t a_size, uint32_t b_size, uint32_t *a, uint32_t *b, uint32_t *c) {
    ...
    for (int32_t i = a_size + b_size - 1; i >= 0; i--) {
        ...
        c[i] = v % NTT32_DECDIG_BASE;
        carry = v / NTT32_DECDIG_BASE;
    }
    ruby_xfree(ntt1);
    ruby_xfree(ntt2);
    ruby_xfree(ntt3);
