...
#define UINT_DBL uint128_t
#else
#define NTT_FUNCNAME(name) name##32
#define UINT uint32_t
#define UINT_DBL uint64_t
#endif
...
MOD_POW(UINT base, UINT ex, UINT mod) {
    ...
    while (true) {
        if (ex & bit) {
            ex ^= bit;
            res = ((UINT_DBL)res * base) % mod;
        }
        if (!ex) break;
        base = ((UINT_DBL)base * base) % mod;
        bit <<= 1;
    }
    return res;
}
...
NTT_RECURSIVE(int size_bits, UINT *input, UINT *output, UINT *tmp, int depth, UINT r, UINT prime) {
    if (depth > 0) {
        NTT_RECURSIVE(size_bits, input, tmp, output, depth - 1, ((UINT_DBL)r * r) % prime, prime);
    } else {
        tmp = input;
    }
    ...
    for (UINT i = 0; i < n; i++) {
        ...
        for (UINT k = 0; k < stride; k++, j++, idx++) {
            ...
            output[idx] = (a + (UINT_DBL)rn * b) % prime;
            output[idx + size_half] = (a + (UINT_DBL)rm * b) % prime;
        }
        rn = ((UINT_DBL)rn * r) % prime;
        rm = ((UINT_DBL)rm * r) % prime;
    }
}
...
NTT(int size_bits, UINT *input, UINT *output, UINT *tmp, int r_base, int base, int shift, int dir) {
    ...
    if (dir < 0) {
        ...
        for (UINT i = 0; i < size; i++) {
            output[i] = ((UINT_DBL)output[i] * n_inv) % prime;
        }
    }
}
