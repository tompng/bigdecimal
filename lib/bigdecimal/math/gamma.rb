...
module BigMath
  ...
  #   Calculating factorial(n + 0.5) is calculated by an optimized BSM version of Lagrange interpolation in quasi-linear time.
  ...
  module Gamma # :nodoc:
    ...
    def self.gamma(x, prec)
      ...
      if x < 0.5
        ...
        base.mult(
          bsm_prod(1..small_factorial_arg, prec2),
          prec2
        ).mult(
      ...
      if x < 0.5
        ...
        if x > prec2 && x.exponent > Integer.sqrt(prec2) / 6
          ...
          lgamma = BigMath.log(base, prec2).add(
            BigMath.log(bsm_prod(1..small_factorial_arg, prec2), prec2),
            prec2
          ).add(
    ...
    # Estimate l that controls the required number of interpolation points `l` to achieve `prec` digits.
    #
    # The function b^x/x! approximates a Gaussian e^(-y^2 / 2b) around its peak (x=b).
    ...
    def self.gamma_lagrange_l(b, prec)
      # Initial guess of l. When b >= 2 * prec, this initial guess is larger than the actual l.
      l = prec
      # Solves the implicit equation by iteration.
    ...
    def self.gamma_lagrange(x, prec) # :nodoc:
      # Shift x to ensure all 2*l + 1 interpolation nodes strictly stay in the positive domain.
      # While x > 1.3 * prec is the strict minimum to avoid poles at x <= 0 (since l <= prec),
      # we heuristically use 2 * prec. A slightly larger base flattens the b^x/x! curve,
      # will reduce `l` and also reduce total computation cost.
      # Required `l` for `x = 2 * prec` is about `prec`.
      ...
      # When x has many significant digits, there will be `l * x.n_significant_digits / prec` full-digit multiplications in BSM.
      # If the total cost exceeds `prec * l`, which is the cost of BSGS, we should use BSGS.
      if x.n_significant_digits * prec.bit_length > prec
        ...
        # Normal BSGS uses batch_size = sqrt(l), but in this case, the coefficients of
        # expanded prod { x - k } for each batch will be huge if batch_size is large.
        # batch_size = log2(prec) is enough to make the BSGS cost to be O(prec * l).
    ...
    def self.gamma_lagrange_n_plus_half(n, prec) # :nodoc:
      ...
      while fractions.size > 1
        fractions = fractions.each_slice(2).map do |a, b|
          ...
          v1 = a[1] * b[1]
          v2 = a[2] * b[2]
          # Drop lower bits to avoid denominator growing too large. It only needs prec * log2(10) bits.
          # 10 / 3 is an approximation of log2(10) with some safety margin
          s = v2.bit_length - prec * 10 / 3
          if s > 0
            v0 >>= s
            v1 >>= s
            v2 >>= s
