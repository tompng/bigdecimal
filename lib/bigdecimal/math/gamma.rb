...
module BigMath
  ...
  #   Only used when x is large enough.
  ...
  module Gamma # :nodoc:
    ...
    def self.gamma(x, prec)
      ...
      if x < 0.5
        ...
        base.mult(
          prod_range(1..small_factorial_arg, prec2),
          prec2
        ).mult(
      ...
      if x < 0.5
        ...
        if x > prec2 && x.exponent > Integer.sqrt(prec2) / 6
          ...
          lgamma = BigMath.log(base, prec2).add(
            BigMath.log(prod_range(1..small_factorial_arg, prec2), prec2),
            prec2
          ).add(
    ...
    # Calculate range.reduce(:*) in a given precision by binary splitting method.
    def self.prod_range(range, prec)
    ...
    def self.integer_factorial(n, prec)
      power_part, exp2, expsqrtpi = integer_factorial_parameter(n, prec)
    ...
    def self.integer_factorial_log(n, prec)
      ...
      if expsqrtpi != 0
        pi = BigMath::PI(prec)
        ans = ans.sub(BigMath.log(pi, prec) * (BigDecimal(expsqrtpi) / 2), prec)
      end
      ans
    end

    # Calculates parameters for integer factorial calculation.
    # Returns [base_power_part, exp2, expsqrtpi] that can produce factorial(n) as:
    #   factorial(n) = prod { base_power_part[i]**(1 << i) } * 2**exp2 / sqrt(pi)**expsqrtpi
    def self.integer_factorial_parameter(n, prec)
      base_power_part, factorial_power_part, exp2, expsqrtpi = integer_factorial_recursive(n, prec)
      fact_x = 1
      fact_y = BigDecimal(1)
      factorial_power_part.each_with_index do |factorial_arg, index|
        fact_y = fact_y.mult(prod_range(fact_x + 1..factorial_arg, prec), prec)
    ...
    def self.integer_factorial_recursive(n, prec)
      if n < 4 * prec
        return [[prod_range(1..n, prec)], [], 0, 0]
      end
      ...
      base = base.mult(prod_range(large_factorial_arg + 1..n / 2, prec), prec)
      base_power_part, factorial_power_part, exp2, expsqrtpi = integer_factorial_recursive(large_factorial_arg, prec)
      [
    ...
    # Specialized version of gamma_lagrange(n + 0.5, prec) for integer n. n > 2 * prec is required.
    ...
    # Precondition: x > prec * ln(10) / 2 / pi
